<template>
  <div class="fc bg-white py-4 px-2 rd dark:bg-hex-16181c dark:text-gray-4">
...
const opacity = (idx: number) => {
  ...
  if (target && target.total > 0) {
    return `background-color:teal;opacity:${target.total / 10}`
  }
  return ''
}
</script>
...
.custom-grid {
  ...
  .grid-item {
    ...
    @apply w-5 h-5 cursor-pointer hover:bg-gray-400 bg-gray-200 rd dark:bg-gray-6;
  }
}
</style>
