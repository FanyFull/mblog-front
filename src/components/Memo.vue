<template>
  <div class="memo" :class="{ top: props.memo.priority > 0 && route.path === '/' }">
    <div class="header">
      ...
      <div v-if="sessionStorage.OPEN_LIKE" class="fr items-center cursor-pointer hover:text-red-400 gap-1 lt-md:hidden"
      ...
      <div v-if="sessionStorage.OPEN_COMMENT" class="fr items-center gap-1 cursor-pointer hover:text-red-400 lt-md:hidden"
      ...
      <div class="fr gap-1 items-center cursor-pointer hover:text-blue-600 ml-auto"
        @click="navTo('/memo/' + props.memo.id)" title="查看详情" v-if="route.path === '/' && !userinfo.token">
      ...
      <n-popover trigger="manual" placement="left" :show="popoverShow" @clickoutside="popoverShow = false"
        ...
        <div class="fc gap-1">
          <div v-if="props.memo.userId === userinfo.userId"
            class="fr gap-1 items-center cursor-pointer hover:text-blue-600" @click="editMemo()">
            <div class="i-carbon:edit"></div>
            <div>编辑</div>
          </div>
          <div class="fr gap-1 items-center cursor-pointer hover:text-blue-600" v-if="route.path === '/'"
            @click="navTo('/memo/' + props.memo.id)">
            <div class="i-carbon:book"></div>
            <div>详情</div>
          </div>
          <div v-if="props.memo.priority === 0 && userinfo.role === 'ADMIN'"
          ...
          <div v-if="userinfo.role === 'ADMIN' && props.memo.priority > 0"
            class="fr gap-1 items-center cursor-pointer hover:text-blue-600"
            @click="setMemoPriority(props.memo.id, false)">
            <div class="i-carbon:down-to-bottom"></div>
            <div>取消置顶</div>
          </div>
          <n-popconfirm :show-icon="false" v-if="props.memo.userId === userinfo.userId || userinfo.role === 'ADMIN'"
            @positive-click="removeMemo(props.memo.id)" negative-text="取消" positive-text="确定">
            <template #trigger>
    ...
    <div ref="el" class="content md-content" :style="{ 'max-height': maxHeight }"
      v-html="props.memo && props.memo.content && marked.parse(props.memo.content)"></div>

    <div class="fr gap-2 px-2 mb-2 flex-wrap" v-if="props.memo.resources">
      <n-image-group>
        <n-space>
          <n-image v-for="(img, index) in props.memo.resources" class="rd hover:shadow-2xl" :key="index" width="100"
            height="100" lazy :src="img.url + (img.suffix || '')" :preview-src="img.url" :intersection-observer-options="{
    ...
    <template v-if="!route.path.startsWith('/memo/')">
      <div v-if="height >= props.maxHeight && fold"
        class="fr justify-center my-4 cursor-pointer hover:text-gray-5 items-center text-gray-4 select-none" title="展开"
        @click="toggleContent">
        <div class="i-carbon:arrow-down"></div>
        <div>展开</div>
      </div>
      <div v-if="!fold"
        class="fr justify-center my-4 cursor-pointer hover:text-gray-5 items-center text-gray-4 select-none" title="折叠"
        @click="toggleContent">
...
.memo {
  ...
  @apply fc bg-white rd dark:bg-hex-282c37 dark:text-hex-eaebf2 mb-2;
  ...
  .header {
    ...
    @apply bg-gray-50 fr gap-2 text-xs py-2 px-4 text-gray-500 rd-t dark:bg-hex-1f232b dark:text-hex-9baec8;
